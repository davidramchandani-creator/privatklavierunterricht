...
import { deleteArchivedInvoices } from "@/app/admin/actions";

export default function BulkDeleteArchivedButton({ count }: { count: number }) {
  ...
  if (!confirm) {
    return (
      ...
      >
        ...
        Alle archivierten löschen ({count})
      </button>
    );
  }
  ...
  return (
    <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-lg px-3 py-1.5">
      <span className="text-xs text-red-700">{count} archivierte Rechnungen endgültig löschen?</span>
      <button
        onClick={() => {
          startTransition(async () => {
            await deleteArchivedInvoices();
            router.refresh();
          });
        }}
        disabled={isPending}
