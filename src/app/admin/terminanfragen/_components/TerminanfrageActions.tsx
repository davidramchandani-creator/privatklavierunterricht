...
export default function TerminanfrageActions({ requestId }: { requestId: string }) {
  ...
  function handleReject() {
    const reason = prompt("Optionaler Ablehnungsgrund (wird dem Schüler mitgeteilt):") ?? undefined;
    setError(null);
    startTransition(async () => {
      const result = await rejectBookingRequest(requestId, reason);
      if (result?.error) setError(result.error);
      else router.refresh();
    });
  }
